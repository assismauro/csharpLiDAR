...
namespace LiDARFileStuff
{
    ...
    public class WDPFile
    {
        ...
        public void GetNextRec(long FWFOffset, int recordSize)
        {
            ...
            Data = new int[recordSize / 2 + 1];              // Record size is in bytes, and data recs are integer
    ...
    public class LiDARFile
    {
        ...
        public ulong LegacyNumberOfPointRecords
        {
            get; set;
        }
        public ulong[] LegacyNumberOfPointbyReturn 
        {
            get; set;
        }
        ...
        {
            LASFileName = fName;
            br = new BinaryReader(File.Open(LASFileName, FileMode.Open,FileAccess.Read));
            ...
            LegacyNumberOfPointbyReturn = new ulong[5];
            for (int i = 0; i < 5; i++)
        ...
        {
            string ret = string.Empty;
            for (ulong i = 0; i < NumberOfVariableLengthRecords; i++)
            {
...
", GetGlobalDecodedString(), StartOfWaveFormDataPacketRecord, GetVariableLengthRecs(),br.BaseStream.Position);
            ...
            for (int i=0; i < points2Print;i++)
            {
                ...
                p.GetFieldValues();
                ret += p.DataAsString + Environment.NewLine;
            }
            return ret;
        }
